...
export interface IContactsClient {
    ...
    create(command: CreateContactCommand): Promise<number>;

    getWithPagination(pageNumber: number | undefined, pageSize: number | undefined, orderBy: string | null | undefined, isOrderByAsc: boolean | undefined): Promise<PaginatedListOfContactBriefDto>;
...
export class ContactsClient implements IContactsClient {
    ...
    protected processGetWithPagination(response: Response): Promise<PaginatedListOfContactBriefDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            result200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver) as PaginatedListOfContactBriefDto;
            return result200;
            });
        } else if (status !== 200 && status !== 204) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<PaginatedListOfContactBriefDto>(null as any);
    }
